...
export default function errorHandler(err, req, res, next) {
  ...
  if (err.type === 'entity.parse.failed') {
    return res.status(err.status).json(error(errCodes.INVALID_JSON, 'Invalid Json'));
  }

  if (err.code === 'BAD_REQUEST') {
    return res.status(err.status).json(error(errCodes.BAD_REQUEST, 'Bad Request'));
  } // for client errors, triggered by throwing new BadRequestError(message)

  return res.status(500).json(error(errCodes.INTERNAL_ERROR, 'server error')); 
}   
